...
async def take_order(callback: types.CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        cursor.execute(
            """UPDATE orders 
            SET status = 'В работе', ph_id = %s 
            WHERE id = %s""",
            (ph_id, order_id)
        )
        
        cursor.execute(
            "SELECT ph_id, message_id FROM order_messages WHERE order_id = %s",
            (order_id,)
        )
...
async def finish_photos_upload(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        cursor.execute(
            "SELECT up.tg_id, om.message_id FROM users_ph up LEFT JOIN order_messages om ON om.ph_id = up.id")
        all_messages = cursor.fetchall()
        print(all_messages)
        ...
        if media:
            ...
            #await bot.send_media_group(chat_id=512471110, media=media)
